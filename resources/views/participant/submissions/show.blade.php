...
<html lang="ru">
    <head>
        ...
        <title>Заявка #{{ $submission->id }} – Кабинет участника</title>
            ...
            <header class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if (in_array($submission->status, ['draft', 'needs_fix']))
                        <a
                            href="{{ route('participant.submissions.edit', $submission) }}"
                ...
                <section class="md:col-span-2 space-y-5">
                    ...
                    <div class="bg-white/90 backdrop-blur shadow-sm rounded-2xl border border-slate-200 p-5">
                        <h2 class="text-sm font-semibold text-slate-900 mb-3">
                            Файлы ({{ $submission->attachments->count() }}/3)
                        </h2>
                                ...
                                @foreach ($submission->attachments as $attachment)
                                    <div class="flex items-center justify-between p-2 border border-slate-200 rounded-lg">
                                        ...
                                        @if ($attachment->status === 'scanned')
                                            <a
                                                href="{{ route('participant.submissions.downloadAttachment', $attachment) }}"
                        ...
                        @if ($submission->attachments->count() < 3 && in_array($submission->status, ['draft', 'needs_fix']))
                            <form method="POST" action="{{ route('participant.submissions.uploadAttachment', $submission) }}" enctype="multipart/form-data" class="border-t border-slate-200 pt-3">
                                @csrf
                                <div class="flex items-center gap-2">
                                    <input
                                        type="file"
                                    ...
                                    <button
                                        type="submit"
                                        class="px-3 py-1.5 text-xs font-semibold rounded-full bg-emerald-600 text-white hover:bg-emerald-500"
                                    >
                                        Загрузить
                                    </button>
                                </div>
                                <p class="mt-1 text-[11px] text-slate-500">
                                    Разрешены: PDF, ZIP, PNG, JPG (максимум 10MB)
                                </p>
                            </form>
                        @endif
                    </div>
                    ...
                    <div class="bg-white/90 backdrop-blur shadow-sm rounded-2xl border border-slate-200 p-5">
                        ...
                        <form method="POST" action="{{ route('participant.submissions.addComment', $submission) }}" class="border-t border-slate-200 pt-3">
                            @csrf
                            <textarea
                                name="body"
                                rows="2"
                ...
                <section class="space-y-5">
                    <div class="bg-white/90 backdrop-blur shadow-sm rounded-2xl border border-slate-200 p-5">
                        ...
                        <div class="space-y-2">
                            @if (in_array($submission->status, ['draft', 'needs_fix']))
                                @php
                                    $scannedCount = $submission->attachments->where('status', 'scanned')->count();
                                @endphp
                                @if ($scannedCount >= 1)
                                    <form method="POST" action="{{ route('participant.submissions.submit', $submission) }}">
                                        @csrf
                                        <button
                                            type="submit"
                                            class="w-full px-3 py-2 text-xs font-semibold rounded-xl bg-emerald-600 text-white hover:bg-emerald-500"
                                        >
                                ...
                                @else
                                    <p class="text-xs text-slate-500">
                                        Для отправки необходимо загрузить хотя бы один файл, прошедший проверку.
                                    </p>
                                @endif
                            @elseif ($submission->status === 'submitted')
                                <p class="text-xs text-slate-500">
                                    Заявка отправлена на проверку жюри.
                                </p>
